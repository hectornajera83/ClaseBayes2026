...
En `brms` la especificación de efectos aleatorios se hace como en el paquete `lmer`: 

- Ordenadas al origen: (1|UNIDAD DEL EFECTO ALEATORIO). 
- Pendientes: (1 + pendiente | UNIDAD DEL EFECTO ALEATORIO)

Comenzaremos por añadir efector aleatorios considerando las ordenadas al origen entre estados. El *híperprior* `set_prior("normal(0,5)", class = "sd") ` especifica la varianza de las ordenadas al origen de `ent`. 

Si se quisieran ordenadas al origen de entidades especificas es necesario hacer la especificación en Stan: `alpha[3] ~ normal(2.0, 0.2)`

```{r}
M1.b<-brm(pobreza ~ 1 + factor(niv_edexpjh) + tamhogesc + factor(tam_loc) + (1 | ent), 
        prior = c(set_prior("normal(0,10)", class = "b"),
                  set_prior("normal(0,5)", class = "sd")),
        family="bernoulli", chains=4, cores=14, iter=1500,  threads = threading(14, grainsize = 100),backend = "cmdstanr", data=D)
summary(M1.b)
```
...
Ahora quisieramos ver si el efecto de la educación varía entre estados. Para ello usaremos: `(1 + niv_edexpjh | ent)`

```{r}
M1.c<-brm(pobreza ~ 1 + factor(niv_edexpjh) + tamhogesc + factor(tam_loc) + (1 + niv_edexpjh | ent), 
        prior = c(set_prior("normal(0,10)", class = "b"),
                  set_prior("normal(0,5)", class = "sd")),
        family="bernoulli", chains=2, cores=14, iter=1500,  threads = threading(14, grainsize = 100),backend = "cmdstanr", data=D)
summary(M1.c)
```
...
```{r}
M1.c.loo<-loo_subsample(M1.c)
loo_compare(M1.b.loo, M1.c.loo)
```

## Modelo de dos tres niveles

```{r}
M1.d<-brm(pobreza ~ 1 + factor(niv_edexpjh) + tamhogesc + factor(tam_loc) + (1 | ent / mun), 
        prior = c(set_prior("normal(0,10)", class = "b"),
                  set_prior("normal(0,5)", class = "sd")),
        family="bernoulli", chains=2, cores=14, iter=1500,  threads = threading(14, grainsize = 100),backend = "cmdstanr", data=D)
summary(M1.d)
...
```{r}
M1.d.loo<-loo_subsample(M1.d)
loo_compare(M1.c.loo, M1.d.loo)
```
...
M2<-brm(i_privacion1 ~ 1 + factor(niv_edexpjh) + tamhogesc + factor(tam_loc) + (1 | ent), 
        prior = c(set_prior("normal(0,10)", class = "b"),
                  set_prior("normal(0,5)", class = "sd")),
        family=cumulative("logit", threshold="flexible"), chains=2, cores=14, iter=2000,  threads = threading(14, grainsize = 100),backend = "cmdstanr", data=D)
summary(M2)
```
